...
exports.deleteCategory = exports.checkCategoryRelationships = exports.updateCategory = exports.createCategory = exports.getCategoryBySlug = exports.getCategoryById = exports.getCategories = void 0;
...
const createCategory = async (req, res) => {
    ...
    `;
        ...
        // Log activity
        await (0, activityLogController_1.logActivity)(req, 'create', 'category', id, name, `Created category "${name}"`);
...
const updateCategory = async (req, res) => {
    ...
    `;
        ...
        // Log activity
        await (0, activityLogController_1.logActivity)(req, 'update', 'category', id, updatedCategory.name, `Updated category "${updatedCategory.name}"`);
...
const checkCategoryRelationships = async (req, res) => {
    try {
        const { id } = req.params;
    ...
    `;
        const productsResult = await database_1.default.query(productsQuery, {
            replacements: { id },
            type: sequelize_1.QueryTypes.SELECT
        });
      ...
      WHERE parent_id = :id
    `;
        const subcategoriesResult = await database_1.default.query(subcategoriesQuery, {
            replacements: { id },
            type: sequelize_1.QueryTypes.SELECT
        });
...
exports.checkCategoryRelationships = checkCategoryRelationships;
const deleteCategory = async (req, res) => {
    ...
    `;
        const relationships = await database_1.default.query(relationshipsQuery, {
            replacements: { id },
            type: sequelize_1.QueryTypes.SELECT
        });
        ...
        const categoryResult = await database_1.default.query(getCategoryQuery, {
            replacements: { id },
            type: sequelize_1.QueryTypes.SELECT
        });
        const categoryName = categoryResult[0]?.name || 'Unknown';
        const result = await database_1.default.query('DELETE FROM product_categories WHERE id = :id RETURNING *', {
            replacements: { id },
        ...
        // Log activity
        await (0, activityLogController_1.logActivity)(req, 'delete', 'category', id, categoryName, `Deleted category "${categoryName}"`);
