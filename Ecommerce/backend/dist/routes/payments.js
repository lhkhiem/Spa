...
const zalopay_1 = require("../services/zalopay");
...
router.post('/zalopay/create', async (req, res) => {
    ...
    `;
      ...
      LIMIT 1
    `;
        const customerData = await database_1.default.query(customerQuery, {
        ...
        const { body, response, app_trans_id } = await (0, zalopay_1.createZaloPayOrder)({
...
router.post('/zalopay/callback', async (req, res) => {
    try {
        ...
        // Verify MAC
        const macOk = (0, zalopay_1.verifyCallbackMac)(data, mac);
        let parsed;
        try {
    ...
    `;
      ...
      `;
            updateResult = await database_1.default.query(updateQuery, {
                replacements: {
                    order_id: order.id,
 ...
 */
router.get('/zalopay/query/:appTransId', async (req, res) => {
    try {
    ...
    `;
        ...
        let queryResult;
        try {
            queryResult = await (0, zalopay_1.queryZaloPayOrder)(appTransId);
        }
        catch (queryError) {
 ...
 */
router.post('/zalopay/refund', async (req, res) => {
    try {
    ...
    `;
        ...
        // Refund via ZaloPay
        const refundResult = await (0, zalopay_1.refundZaloPayTransaction)(order.zp_trans_id, refundAmount, description || `Hoàn tiền đơn hàng ${order.order_number}`);
 ...
 */
router.get('/zalopay/refund/query/:mRefundId', async (req, res) => {
    try {
        ...
        const refundStatus = await (0, zalopay_1.queryZaloPayRefund)(mRefundId);
        return res.json({
            success: true,
            data: refundStatus
        });
