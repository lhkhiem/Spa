...
async function runMigration() {
  ...
  try {
    ...
    const migrationPath = path.join(__dirname, 'src/migrations/034_create_product_images_attributes.sql');
    if (!fs.existsSync(migrationPath)) {
      throw new Error(`Migration file not found: ${migrationPath}`);
    }
    const sql = fs.readFileSync(migrationPath, 'utf8');

    console.log('📄 Running migration: 034_create_product_images_attributes.sql');
    console.log('   This will create product_images and product_attributes tables\n');
    ...
    console.log('   - Migrated data from product_group_images to product_images');
    console.log('   - Created product_attributes table');
    console.log('   - Migrated data from product_group_attributes to product_attributes');
