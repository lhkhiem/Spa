...
async function seed() {
    ...
    `);
        ...
        for (const asset of assetSeeds) {
            const existing = await pool.query(`SELECT id FROM assets WHERE url = $1 LIMIT 1`, [asset.url]);
        ...
        `, [
          ...
          `, [productId, categoryId]);
            ...
            if (product.galleryKeys && product.galleryKeys.length > 0) {
                const existingImages = await pool.query(`SELECT COUNT(*)::int AS count FROM product_images WHERE product_id = $1`, [productId]);
        ...
        const testimonials = [
            {
                ...
                testimonial_text: 'I have been dealing with Digital PressUp for over 10 years and have never been disappointed in a product or their great customer service. Thank you for making my job easier as a Clinic Esthetician.',
                rating: 5,
                sort_order: 0,
                is_featured: true,
            },
            {
                ...
                testimonial_text: 'All your needs in one place like being at a party store! Amazing selection and fast shipping. Highly recommend Digital PressUp!',
                rating: 5,
                sort_order: 1,
                is_featured: true,
            },
            {
                ...
                testimonial_text: 'Outstanding quality and service! The educational resources have been invaluable for my team. We trust Digital PressUp for all our spa supplies.',
                rating: 5,
                sort_order: 2,
                is_featured: true,
            },
        ...
        RETURNING id
      `);
        const learningTopicId = learningTopicResult.rows[0]?.id ||
        ...
        const adminResult = await pool.query(`SELECT id FROM users WHERE email = 'admin@pressup.com' LIMIT 1`);
        ...
        const learningPosts = [
            {
                ...
                excerpt: 'Soft clients will beat a path to your door for beautiful brows. Learn five reasons to add lash and brow services to your menu.',
                content: {
                    type: 'doc',
                    content: [
                        {
            ...
            },
            {
                ...
                excerpt: 'Balancing the physical and emotional demands of massage therapy can be challenging. Discover techniques to stay energized and inspired.',
                content: {
                    type: 'doc',
                    content: [
                        {
