...
exports.resetDefaults = exports.clearCache = exports.putNamespace = exports.getNamespace = void 0;
...
const getNamespace = async (req, res) => {
    try {
        ...
        if (namespace === 'seo') {
            try {
                const PageMetadata = (await Promise.resolve().then(() => __importStar(require('../models/PageMetadata')))).default;
        ...
        const row = await database_1.default.query('SELECT value FROM settings WHERE namespace = :ns', {
            type: 'SELECT',
            replacements: { ns: namespace },
        });
        const val = row[0]?.value ?? DEFAULTS[namespace] ?? {};
...
const putNamespace = async (req, res) => {
    try {
        ...
        if (namespace === 'seo') {
            ...
            if (value.pages && Array.isArray(value.pages)) {
                ...
                try {
                    const { syncSeoSettingsToPageMetadata } = await Promise.resolve().then(() => __importStar(require('../utils/syncSeoSettingsToPageMetadata')));
        ...
        try {
         ...
         ON CONFLICT (namespace) DO UPDATE SET value = EXCLUDED.value, updated_at = NOW()`, { type: 'INSERT', replacements: { ns: namespace, val: jsonValue } });
            console.log('[putNamespace] Insert/Update completed');
            // Verify the save by reading back
            const verifyResult = await database_1.default.query('SELECT value FROM settings WHERE namespace = :ns', {
                type: 'SELECT',
                replacements: { ns: namespace },
            });
        ...
        try {
            await (0, activityLogController_1.logActivity)(req, 'update', 'settings', namespace, namespace, `Updated settings: ${namespace}`);
        }
        catch (activityError) {
...
exports.putNamespace = putNamespace;
...
const resetDefaults = async (req, res) => {
    try {
        ...
        await database_1.default.query(`INSERT INTO settings (namespace, value, updated_at)
       VALUES (:ns, :val::jsonb, NOW())
       ON CONFLICT (namespace) DO UPDATE SET value = EXCLUDED.value, updated_at = NOW()`, { type: 'INSERT', replacements: { ns: scope, val: JSON.stringify(defaults) } });
