...
# Script to rebuild and deploy CMS Backend to Production
# Usage: ./rebuild-deploy-cms-backend.sh
...
# Directories
SOURCE_DIR="/var/www/Spa/CMS/backend"
PUBLISH_DIR="/var/www/publish/spa/cms-backend"

echo -e "${BLUE}=== Rebuild and Deploy CMS Backend ===${NC}\n"
echo -e "${YELLOW}Source: ${SOURCE_DIR}${NC}"
echo -e "${YELLOW}Destination: ${PUBLISH_DIR}${NC}\n"

# Step 1: Build
echo -e "${YELLOW}[1/4] Building CMS Backend...${NC}"
cd "$SOURCE_DIR"
...
# Note: storage/uploads is not copied (should be managed separately)
...
if pm2 list | grep -q "cms-backend"; then
    pm2 restart cms-backend
    if [ $? -eq 0 ]; then
        echo -e "${GREEN}✓ Service restarted${NC}\n"
        sleep 2
        pm2 list | grep cms-backend || true
    else
        echo -e "${RED}✗ Failed to restart service${NC}"
        exit 1
    fi
else
    echo -e "${YELLOW}⚠ Service 'cms-backend' not found in PM2. Skipping restart.${NC}"
