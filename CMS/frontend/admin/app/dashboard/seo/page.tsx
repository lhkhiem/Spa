...
export default function SEOPage() {
  ...
  const staticPages = (seo.pages || []).filter((p: any) => !p.autoGenerated && !p.path?.includes(':'));
  const dynamicPages = (seo.pages || []).filter((p: any) => p.autoGenerated || p.path?.includes(':'));

  const renderPageList = (pages: any[], type: 'static' | 'dynamic') => (
    <div className="space-y-4">
      {pages.map((page: any, index: number) => {
        const originalIndex = seo.pages?.findIndex((p: any) => p === page) ?? -1;
        if (originalIndex === -1) return null;

        return (
          <div key={originalIndex} className="border border-border rounded-lg p-4 bg-background">
            {editingPageIndex === originalIndex ? (
              // Edit mode
              <div className="space-y-4">
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Cấu hình metadata (title, description, OG image) cho từng trang để tối ưu khi share lên mạng xã hội</p>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="rounded-lg border border-border bg-card p-6 space-y-6">
            ...
            <button
              onClick={async () => {
                ...
                try {
                  ...
                  toast.error(e.response?.data?.error || 'Tạo trang thất bại');
                }
              }}
        ...
        <div className="space-y-4 border-t border-border pt-6">
          <div>
            <h4 className="text-base font-semibold text-foreground">Trang động</h4>
            <p className="text-xs text-muted-foreground mt-1">
              Các trang tự động tạo từ posts và products (ví dụ: /posts/[slug], /products/[slug]). 
              Metadata được tự động sync khi tạo/sửa. Bạn có thể chỉnh sửa để tùy chỉnh.
            </p>
          </div>
          {dynamicPages.length > 0 ? (
            renderPageList(dynamicPages, 'dynamic')
          ) : (
            <div className="border border-dashed border-border rounded-lg p-8 text-center">
              <p className="text-sm text-muted-foreground">
                Chưa có trang động nào. Các trang này sẽ tự động xuất hiện khi bạn tạo/sửa posts hoặc products.
              </p>
            </div>
          )}
        </div>
