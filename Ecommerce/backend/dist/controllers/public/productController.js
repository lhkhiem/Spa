...
exports.getProductDetail = exports.listProducts = void 0;
const sequelize_1 = require("sequelize");
const database_1 = __importDefault(require("../../config/database"));
const domainUtils_1 = require("../../utils/domainUtils");
const parseCsvParam = (input) => {
    if (!input) {
        return [];
    }
...
const buildPriceConditions = (ranges, replacements) => {
    const conditions = [];
...
const listProducts = async (req, res) => {
    try {
        ...
        if (searchQuery) {
            whereClauses.push(`(p.name ILIKE :search OR p.description ILIKE :search OR p.sku ILIKE :search)`);
    ...
    `;
        ...
        const data = items.map((row) => {
            ...
            const thumbnailUrl = (0, domainUtils_1.normalizeMediaUrl)(row.thumbnail_cdn_url) ||
                (0, domainUtils_1.normalizeMediaUrl)(row.thumbnail_url);
            const price = Number(row.price);
            const comparePrice = row.compare_price !== null && row.compare_price !== undefined
            ...
            return {
                ...
                brand: row.brand_id
                    ...
                    : null,
                categories,
                rating: row.average_rating !== null && row.average_rating !== undefined
...
const getProductDetail = async (req, res) => {
    try {
      ...
      LIMIT 1
    `;
        const productRows = await database_1.default.query(productQuery, {
        ...
        const parsedContent = typeof product.content === 'string'
            ? (() => {
                try {
    ...
    `;
        const [imageRows, attributeRows, relatedRows] = await Promise.all([
            database_1.default.query(imagesQuery, {
                replacements: { productId },
                type: sequelize_1.QueryTypes.SELECT,
            }),
            database_1.default.query(attributesQuery, {
                replacements: { productId },
                type: sequelize_1.QueryTypes.SELECT,
            }),
            database_1.default.query(relatedQuery, {
                replacements: {
                    productId,
        ...
        const detail = {
            ...
            comparePrice: product.compare_price !== null && product.compare_price !== undefined
            ...
            variants: [],
            options: [],
            thumbnailUrl: (0, domainUtils_1.normalizeMediaUrl)(product.thumbnail_cdn_url) ||
                (0, domainUtils_1.normalizeMediaUrl)(product.thumbnail_url),
            ...
            rating: product.average_rating !== null && product.average_rating !== undefined
                ...
                : 0,
            reviewCount: product.review_count !== null && product.review_count !== undefined
                ? Number(product.review_count)
                : 0,
            images: imageRows.map((image) => ({
                id: image.id,
                sortOrder: image.sort_order,
                url: (0, domainUtils_1.normalizeMediaUrl)(image.cdn_url) ||
                    (0, domainUtils_1.normalizeMediaUrl)(image.url),
                width: image.width,
                height: image.height,
                format: image.format,
            })),
            ...
            relatedProducts: relatedRows.map((item) => ({
                ...
                comparePrice: item.compare_price !== null && item.compare_price !== undefined
                    ? Number(item.compare_price)
                    : null,
                isFeatured: Boolean(item.is_featured),
                isBestSeller: Boolean(item.is_best_seller),
                thumbnailUrl: (0, domainUtils_1.normalizeMediaUrl)(item.thumbnail_cdn_url) ||
                    (0, domainUtils_1.normalizeMediaUrl)(item.thumbnail_url),
