...
const createUser = async (req, res) => {
    try {
        ...
        const user = await User_1.default.create({ email, name, password_hash, role: newRole, status: 'active' });
        // Log activity
        await (0, activityLogController_1.logActivity)(req, 'create', 'user', user.id, name, `Created user "${name}" (${email})`);
        res.status(201).json({
            id: user.id,
            email: user.email,
            name: user.name,
...
const updateUser = async (req, res) => {
    try {
        ...
        await (0, activityLogController_1.logActivity)(req, 'update', 'user', id, userName, `Updated user "${userName}" (${userEmail})`);
        res.json({
            id: user.id,
            email: user.email,
            name: user.name,
...
const deleteUser = async (req, res) => {
    try {
        ...
        await user.destroy();
        // Log activity
        await (0, activityLogController_1.logActivity)(req, 'delete', 'user', id, userName, `Deleted user "${userName}" (${userEmail})`);
