...
exports.createZaloPayOrder = createZaloPayOrder;
exports.verifyCallbackMac = verifyCallbackMac;
exports.queryZaloPayOrder = queryZaloPayOrder;
exports.refundZaloPayTransaction = refundZaloPayTransaction;
exports.queryZaloPayRefund = queryZaloPayRefund;
const axios_1 = __importDefault(require("axios"));
const hmac_1 = require("../utils/hmac");
const zalopayTime_1 = require("../utils/zalopayTime");
/**
 * Create ZaloPay order
 * Returns order_url to redirect user for payment
 */
async function createZaloPayOrder(params) {
    ...
    let app_trans_id = `${(0, zalopayTime_1.vnYYMMDD)()}_${orderIdShort}`;
    // ZaloPay requires app_trans_id max 40 chars
    if (app_trans_id.length > 40) {
        const datePrefix = (0, zalopayTime_1.vnYYMMDD)();
    ...
    const mac = (0, hmac_1.hmacSHA256Hex)(key1, macInput);
    ...
    try {
        const { data } = await axios_1.default.post(`${base}${path}`, body, {
 ...
 */
async function queryZaloPayOrder(app_trans_id) {
    const app_id = Number(process.env.ZP_APP_ID);
    const key1 = process.env.ZP_KEY1;
    if (!app_id || !key1) {
    ...
    try {
        const { data } = await axios_1.default.post(`${base}${path}`, requestBody, {
            headers: { 'Content-Type': 'application/json' },
            timeout: 30000,
        });
 ...
 */
async function refundZaloPayTransaction(zp_trans_id, amount, description = 'Hoàn tiền đơn hàng') {
    const app_id = Number(process.env.ZP_APP_ID);
    const key1 = process.env.ZP_KEY1;
    if (!app_id || !key1) {
        throw new Error('ZaloPay configuration missing: ZP_APP_ID or ZP_KEY1');
 ...
 */
async function queryZaloPayRefund(m_refund_id) {
    const app_id = Number(process.env.ZP_APP_ID);
    const key1 = process.env.ZP_KEY1;
    if (!app_id || !key1) {
        throw new Error('ZaloPay configuration missing: ZP_APP_ID or ZP_KEY1');
...
//# sourceMappingURL=zalopay.js.map