...
async function migratePageMetadata() {
  try {
    ...
    if (!seoRow || seoRow.length === 0 || !seoRow[0]?.value) {
      console.log('⚠️  No SEO settings found in settings table');
      return;
    }
    ...
    if (pages.length === 0) {
      console.log('⚠️  No pages found in SEO settings');
      return;
    }

    console.log(`📦 Found ${pages.length} pages in settings to migrate`);
    ...
    for (const page of pages) {
      if (!page || !page.path) {
        console.warn('⚠️  Skipping page without path:', page);
        skipped++;
        continue;
      }
      ...
      try {
        ...
        if (existing) {
          console.log(`⏭️  Page already exists: ${normalizedPath}`);
          skipped++;
          continue;
        }
        ...
        console.log(`✅ Migrated: ${normalizedPath}`);
        migrated++;
      } catch (error: any) {
        console.error(`❌ Error migrating page ${page.path}:`, error.message);
        errors++;
      }
    }
