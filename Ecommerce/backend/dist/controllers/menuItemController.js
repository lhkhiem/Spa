...
exports.updateMenuItemsOrder = exports.deleteMenuItem = exports.updateMenuItem = exports.createMenuItem = exports.getMenuItemById = exports.getMenuItems = void 0;
const sequelize_1 = require("sequelize");
const database_1 = __importDefault(require("../config/database"));
const uuid_1 = require("uuid");
// Helper: Calculate menu item depth (max 3 levels)
async function getMenuItemDepth(parentId) {
    if (!parentId)
        return 1; // Root level
    let depth = 1;
...
async function getSubtreeMaxDepth(itemId) {
    const query = `SELECT id FROM menu_items WHERE parent_id = :itemId`;
    const children = await database_1.default.query(query, {
        replacements: { itemId },
        type: sequelize_1.QueryTypes.SELECT
    });
    ...
    const childrenDepths = await Promise.all(children.map((child) => getSubtreeMaxDepth(child.id)));
    return 1 + Math.max(...childrenDepths);
}
// Helper: Check if item can be moved to new parent without exceeding max depth
async function canMoveToParent(itemId, newParentId) {
    // Calculate new depth
...
const createMenuItem = async (req, res) => {
    try {
        const { menu_location_id, parent_id, title, url, icon, type, entity_id, target, rel, css_classes, sort_order, is_active } = req.body;
...
const updateMenuItem = async (req, res) => {
    try {
        const { id } = req.params;
        const { parent_id, title, url, icon, type, entity_id, target, rel, css_classes, sort_order, is_active } = req.body;
        const updateFields = [];
...
const deleteMenuItem = async (req, res) => {
    try {
        const { id } = req.params;
        const result = await database_1.default.query('DELETE FROM menu_items WHERE id = :id RETURNING *', {
            replacements: { id },
    ...
    try {
        ...
        await database_1.default.transaction(async (transaction) => {
            for (const item of items) {
                await database_1.default.query('UPDATE menu_items SET sort_order = :sort_order, parent_id = :parent_id WHERE id = :id', {
                    replacements: {
                        id: item.id,
...
exports.updateMenuItemsOrder = updateMenuItemsOrder;
