...
# Script to rebuild and deploy Ecommerce Frontend to Production
# Usage: ./rebuild-deploy-ecommerce-frontend.sh
...
# Directories
SOURCE_DIR="/var/www/Spa/Ecommerce/frontend"
PUBLISH_DIR="/var/www/publish/spa/ecommerce-frontend"

echo -e "${BLUE}=== Rebuild and Deploy Ecommerce Frontend ===${NC}\n"
echo -e "${YELLOW}Source: ${SOURCE_DIR}${NC}"
echo -e "${YELLOW}Destination: ${PUBLISH_DIR}${NC}\n"

# Step 1: Build
echo -e "${YELLOW}[1/4] Building Ecommerce Frontend...${NC}"
cd "$SOURCE_DIR"
...
if pm2 list | grep -q "ecommerce-frontend"; then
    ...
    if [ $? -eq 0 ]; then
        ...
        sleep 2
        pm2 list | grep ecommerce-frontend || true
    else
        echo -e "${RED}✗ Failed to restart service${NC}"
        exit 1
    fi
else
    echo -e "${YELLOW}⚠ Service 'ecommerce-frontend' not found in PM2. Skipping restart.${NC}"
...
echo ""
echo "To check logs: pm2 logs ecommerce-frontend --lines 50"
