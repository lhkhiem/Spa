...
async function normalizeMetadataPaths() {
    try {
        ...
        const pathMap = new Map(); // old path -> new path
        const duplicates = []; // paths that would create duplicates
        const normalizedPages = existingPages.map((page) => {
            ...
            if (oldPath !== newPath) {
                ...
                const existingWithNewPath = existingPages.find((p) => p.path === newPath && p !== page);
        ...
        if (duplicates.length > 0) {
            ...
            const seen = new Set();
            const deduplicatedPages = normalizedPages.filter((page) => {
...
//# sourceMappingURL=normalizeMetadataPaths.js.map